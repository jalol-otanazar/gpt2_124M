...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        B, T = idx.size()
        assert T <= self.config.block_size, f"{T} cannot be bigger than context length {self.config.block_size}"
...
class DataLoader:
  ...
  def next_batch(self):
    ...
    # return to 0 if the load goes to the maximum
...
for i in range(50):
    ...
    print(f"step {i} loss {loss.item()} {dt:.2f}ms {token_per_sec:.2f} tokens/s")
